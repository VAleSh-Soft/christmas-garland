...
`#define POWER_I 2000` - ток блока питания в миллиамперах, позволяет ограничить потребляемую мощность гирлянды возможностями блока питания; возможно, это продлит срок службы вашего блока питания )); если ограничение по потребляемому току не нужно, задайте `POWER_I` равным `0` или закомментируйте эту строку;
