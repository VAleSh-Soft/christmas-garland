...
- [Онлайн настройка модуля под гирлянду](#онлайн-настройка-модуля-под-гирлянду)
  ...
  - [Настройка количества светодиодов](#настройка-количества-светодиодов)
...
`#define SAVE_EEPROM` 1 - сохранять настройки запуска в EEPROM (0 - ничего не  хранить 1 - хранить стартовый режим, длину гирлянды, задержку, яркость, очередность цветов в чипе и расширенные настройки (состояние блеска, фона, свечей))
...
*Изменение **EORDER** реализовано не инициализацией гирлянды соответствующим значением, а подменой очередности следования байтов перед их выводом (см. функцию `CRGB set_new_eorder(CRGB _col);` в файле **_leds.h**)*
 ...
 * при включении питания модуля удерживать нажатой кнопку №2; гирлянда включится в режиме настройки количества светодиодов - будут светиться красным все уже заданные светодиоды;
 ...
 * гирлянда включится в режиме настройки вершины - включится заданное количество светодиодов (по умолчанию `TOP_LENGTH`) в конце гирлянды; цвет свечения определяется макросом `#define TOP_COLOR`; остальные светодиоды будут светиться слабым голубым цветом;
 ...
 * если используются четыре кнопки, то клик третьей кнопкой меняет цвет заливки вершины; возможные значения - цвета радуги и белый цвет; если модуль использует менее четырех кнопок, то цвет заливки вершины будет определяться макросом `#define TOP_COLOR`;
 * выключить и снова включить модуль для применения настроек;
