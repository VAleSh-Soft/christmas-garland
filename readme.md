 ...
 * гирлянда включится в режиме настройки вершины - будет светиться только `TOP_LENGTH` светодиодов в конце гирлянды; цвет свечения определяется макросом `#define TOP_COLOR`;
