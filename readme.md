...
*Изменение **EORDER** реализовано не инициализацией гирлянды соответствующим значением, а подменой очередности следования байтов перед их выводом (см. функцию `CRGB set_new_eorder(CRGB _col);` в файле **setting.h**)*
 ...
 * гирлянда включится в режиме настройки вершины - включится заданное количество светодиодов (по умолчанию `TOP_LENGTH`) в конце гирлянды; цвет свечения определяется макросом `#define TOP_COLOR`; остальные светодиоды будут светиться цветом заливки фона (по умолчанию - слабым голубым);
